...
class Voter {
	...
	public function getTalksByCategory($category) {
		return $this->db->aquery("SELECT t.title, t.description, t.type, t.level, v.rating FROM talks t LEFT JOIN votes v ON v.talk_id=t.id AND v.voter_id=? WHERE t.category=? ORDER BY t.title", $this->id, $category);
	}
